  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        ...
        // Fix: Extract URLs from image objects
        setImageUrls(product.images?.map((img) => img.url) || []);

        // Fix: Handle colors properly
        setColors(
        ...
        if (product.specifications) {
          setSpecifications({
            ...
            packContains: product.specifications.packContains || "",
            weight: product.specifications.weight || "",
            dimensions: product.specifications.dimensions || "",
            careInstructions: product.specifications.careInstructions || "",
            safetyFeatures: product.specifications.safetyFeatures || "",
          });
        }
      } catch (error) {
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <form onSubmit={handleSubmit}>
          ...
          <div className="px-6 py-3 bg-gray-50 border-t border-gray-200 flex justify-end space-x-3">
            ...
            <button
              type="submit"
              disabled={isSaving || !imageUrls.length}
